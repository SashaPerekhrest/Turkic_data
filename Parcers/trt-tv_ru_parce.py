...
main_urls = [
    'https://trt-tv.ru/tat/tag/k-nkartinasy/page/'
]
...
output_file = open('2tt_parced.txt', 'a', encoding='utf-8')  # Открываем файл для записи (режим 'a' для добавления данных)
...
for main_url in main_urls:
    ...
    while True:  # Бесконечный цикл
        ...
        news_block = soup.find_all(class_='jeg_postblock_content')
        if len(news_block) == 0:
        ...
        if len(hrefs) != 0:
            ...
            for href in hrefs:
                ...
                main_content_element = soup_one_news.find(class_='entry-content no-share')
                ...
                if main_content != []:
                    for e in main_content:
                        news_text += e.text
                output_file.write(news_text + '###' + href + '###' + main_url + '$$$' + '\n')  # Записываем данные в файл
                j += 1
